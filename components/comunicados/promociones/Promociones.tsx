...
const Promociones: FC<{ selectData: any, dataContentful: any }> = ({ selectData = [], dataContentful }) => {
    ...
    const filters = (item: any) => {
        if (item?.exceptions) {
            ...
            if (
                (item?.exceptions?.showToDistributor.includes(org?.level))
            ) {
                return item;
            } else return false;
        }
        ...
        <div>
            ...
            <div className="flex gap-6 justify-center mt-6">
                {["Todos", ...categorys() as any].map((data) => (
                    <ButtonBgOut
                        title={data}
                        styles={`hover:bg-red-100 hover:!text-red-500 hover:!text-sm ${data === content && "bg-red-100 !text-red-500"
                            }`}
                ...
                <div className="grid grid-cols-3 gap-3 gap-y-6">
                    {data
                        .filter((item: any) => {
                            ...
                            if (content !== "Todos" && !item.category.includes(content)) {
                                return false;
                            }
