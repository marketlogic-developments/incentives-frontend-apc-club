...
const Layout = ({ children }) => {
  ...
  const locations = [
    ...
    {
      page: userRedux.roleId === 1 ? "/digipointsall" : "/digipoints",
      icon: (
        <svg
          width="20"
          height="20"
    ...
    if (userRedux !== 0) {
      return locations
        .filter(({ page }) => {
          ...
          if (n === 3) {
            if (userRedux.roleId === 1) {
              return ["/customercare", "/reglas", "/reportes"].includes(page);
            }
          }
        })
        ...
        <div className="globalContent bg-primary">
          <div className="containerLayout">
            <div className="flex flex-col py-6 gap-6 h-screen">
              ...
              <div className="flex flex-col gap-6 overflow-y-scroll scrollMenu">
                <div className="containerRedirections gap-2">{menu(1)}</div>
                <hr className="mx-6" />
                <div className="containerRedirections gap-2">{menu(2)}</div>
                <hr className="mx-6" />
                <div className="containerRedirections gap-2">{menu(3)}</div>
              </div>
            </div>
          </div>
