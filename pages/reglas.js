...
const premios = () => {
    ...
    const awards = useSelector((state) => state.awards.rules);
                ...
                <table className="w-full text-sm text-left text-black-500">
                    <thead className="text-xs text-black-500 uppercase">
                        <tr>
                            ...
                            <th scope="col" className="py-3 px-6">DigiPoints</th>
                            <th scope="col" className="py-3 px-6">Monto</th>
                            <th scope="col" className="py-3 px-6">Market segment</th>
                            <th scope="col" className="py-3 px-6">Business unit</th>
                            <th scope="col" className="py-3 px-6">Business type</th>
                            <th scope="col" className="py-3 px-6">Columna</th>
                            <th scope="col" className="py-3 px-6">Operador</th>
                            <th scope="col" className="py-3 px-6">Valor</th>
                            <th scope="col" className="py-3 px-6">Major licensing rogram</th>
                            <th scope="col" className="py-3 px-6">Nombre Product</th>
                            <th scope="col" className="py-3 px-6">Named vs Device</th>
                            <th scope="col" className="py-3 px-6">Puntos para Distri</th>
                        </tr>
                    </thead>
                    <tbody>
                        {[...currentItems]
                            ...
                            .map(
                                (data) => (
                                    ...
                                    >
                                        ...
                                        <td className="py-4 px-6">{data.digipointsPerAmount}</td>
                                        <td className="py-4 px-6">{data.baseAmount}</td>
                                        <td className="py-4 px-6">{data.businessUnit}</td>
                                        <td className="py-4 px-6">{data.business_unit}</td>
                                        <td className="py-4 px-6">{data.businessType}</td>
                                        <td className="py-4 px-6">{data.columnName}</td>
                                        <td className="py-4 px-6">{data.operator}</td>
                                        <td className="py-4 px-6">{data.columnValueProcess}</td>
                                        <td className="py-4 px-6">{data.majorLicensingProgram}</td>
                                        <td className="py-4 px-6">{data.productName}</td>
                                        <td className="py-4 px-6">{data.namedVsDevice}</td>
                                        <td className="py-4 px-6">{data.digiPointsPerDistri}</td>
                                    </tr>
                                )
                            )}
                    </tbody>
        ...
        if (modal === 0) {
            return (
                <div className="flex flex-col items-center p-10">
                    <h3 className="text-lg font-bold text-red-500">Agregar Reglas</h3>
                    <p className="py-4">Indica la información de la regla</p>
                    <form
                        onSubmit={(info) => {
                            handleSubmit(info);
                        }}
        ...
        <>
            <Modal opened={opened} onClose={() => setOpened(false)} size={"50%"}>
                <div className="grid grid-cols-1 place-items-center">
                    <p
                        className={`p-3 w-full text-center ${modal === 0 && "border-b-2 border-[#eb1000] text-[#eb1000]"
                            }`}
                        onClick={() => setModal(0)}
                    >
                        Agregar Regla
