...
import { teamsUpdate } from "../../store/reducers/teams.reducer";
...
const MakeTeam = () => {
  ...
  const { ListAllTeams, deleteTeam } = TeamsFunction();
  ...
  const handleDeleteTeam = (data: Team) => {
    Swal.fire({
      title: String(t("digipoints.deleteTeam")),
      text: String(t("digipoints.copyDeleteTeam")),
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#eb1000",
      cancelButtonColor: "#3085d6",
      confirmButtonText: String(t("digipoints.delete")),
  ...
  const dowloadTeams = () => {
    const data = teams?.content.map((item) => {
      let { id, ...info } = item;
      return info;
    });
