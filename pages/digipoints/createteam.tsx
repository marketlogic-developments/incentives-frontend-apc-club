import { Menu, Modal, Skeleton } from "@mantine/core";
...
import { getAllTeams, teamsUpdate } from "../../store/reducers/teams.reducer";
import jsonexport from "jsonexport";
import { saveAs } from "file-saver";
import { RootState } from "store/store";
import ModalCreateTeam from "components/digipoints/MakeTeam/ModalCreateTeam";
...
const MakeTeam = () => {
  ...
  function compararFechasAscendente(a: string, b: string) {
    const fechaA = new Date(a.created_at.split("/").reverse().join("/"));
    const fechaB = new Date(b.created_at.split("/").reverse().join("/"));
  ...
  // Función de comparación personalizada para ordenar de forma descendente (de más reciente a más antiguo)
  ...
  const dowloadTeams = () => {
    const data = teams.map((item) => {
      let { id, partner_admin_id, ...info } = item;
      return info;
    });
        ...
        <div className="w-full">
          <div className="overflow-x-auto">
            <table className="w-full text-sm text-left text-black-500 overflow-hidden rounded-md">
              ...
              <tbody>
                {teams?.lenght === 0 ? (
                  <Skeleton visible={true} height={8}>
                    {[...teams]
                      .sort(
                        selectDate === "upDown"
                                ...
                                <Menu.Dropdown>
                                  ...
                                  >
                                    <div className="flex gap-6 items-center">
                                      ...
                                      <p className="text-[#eb1000]">Editar</p>
                                    </div>
                                  </Menu.Item>
                                  <Menu.Item
                                    onClick={() => handleDeleteTeam(data)}
                                  >
