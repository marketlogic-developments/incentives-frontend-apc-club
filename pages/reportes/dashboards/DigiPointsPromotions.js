...
const DigiPointsPromotions = () => {
  ...
  useEffect(() => {
    if (isLoaded && token) {
      setLoading(true);
      dispatch(getDigiPointsByPromotions(token))
        .then((response) => {
          setLoading(false);
  ...
  const importFile = async (data) => {
    const columns = digiPointsByPromotionsCsv(data);
    const csvConfig = {
      data: data,
      columns: columns,
      downloadTitle: "Promo Report",
    };
  ...
  const importFileExcel = async (data) => {
    const excelConfig = {
      data: data,
      columns: digiPointsByPromotionsExel,
      downloadTitle: "Promo Report",
    };
  ...
  return (
    <div className="mt-8">
      <div className="grid grid-rows-1">
        <TitleWithIcon icon={<Promotion />} title={"Promo Report"} />
      </div>
      <div className="flex w-full items-center gap-4 pt-10 pb-2 pl-0">
        ...
        <span className="font-bold text-[#1473E6]">{"Promo Report"}</span>
      </div>
      <div className="grid items-center sm:grid-cols-3 grid-cols-2 gap-3">
        <SelectInputValue
            ...
            cols={[
              {
                rowStyles: "",
                symbol: "",
                identity: "promotions_name",
                columnName: "Promotions Name",
              },
              {
                symbol: "",
                sort: true,
...
export default DigiPointsPromotions;
