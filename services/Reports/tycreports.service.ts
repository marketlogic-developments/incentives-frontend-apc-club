...
import { GenericalPromise, HandleError, MultipleElements } from "services/generical.service";
...
export const ReportTyCPerChannelPPPA = async (params: string) => {
  try {
    const response = await API.get<GenericalPromise<MultipleElements<CompaReportTyCPerChannelPPPAPropsny>>>(
